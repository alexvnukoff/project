...
from usersites.B2BProducts.views import  B2BProductListDetail
...
urlpatterns = [
    ...
    url(r'^(?P<slug>[a-zA-z0-9-]+)-(?P<pk>[0-9]+)?/$', render_page,
        kwargs={'template': 'B2BProducts/contentPage.html', 'title': _("B2B Products")}, name="category"),
    url(r'^(?P<slug>[a-zA-z0-9-]+)-(?P<pk>[0-9]+)?/page(?P<page>[0-9]+)?/$', render_page,
