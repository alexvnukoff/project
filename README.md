...
#####Правила при разработке
 ...
 + Удалять обьекты можно только по одному (bulk нельзя) для переиндексации и удаления связаных обьектов
...
Группы
 ...
 + Redactor - Может добовлять ТВ новости и новости на главную.
 + TPP Creator - Может создавать новые ТПП
...
У каждой модели есть 3 вида разрешений
 ...
 + Add (напр. add_company для модели Company) - Пользователь может добовлять обьекты модели
 + Change  (напр. change_ business _proposal для модели BusinessProposal) - Пользователь может изменять обьекты модели
 + Delete - Пользователь может удалять обьекты модели
...
у каждого пользователя есть 5 флагов
 ...
 + is_superuser - Супер пользователь(Для Django)
 + is_commando - Тоже что и супер пользователь но не имеет прав в Django admin

######Типы связязей

Иерархия связей в системе немного запутана, есть 3 основных типа начальных точек

 + Dependence - Обьект принадлежит другому обьекту и зависит от него во всех отношениях (компания - продукт) могут иметь только одну связь к родителю.
 + Relation - Обьект просто связан с другим обьектом (продукт - категория) и могут иметь много связий.
 + Hierarchy - У обьекта могут быть иерархические связи (категория - под-категория) могут иметь только одну связь к родителю.
...
Модель безопасности позволяет модераторам (работникам) ТПП управлять компаниями которые связаны с этой ТПП и их обьектами,
Сотрудники компаний в свою очередь могут управлять только компанией и её обьектами(Продукты, предложения и т.д).
