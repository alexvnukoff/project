...
urlpatterns = [
    ...
    url(r'^category/(?P<category>[0-9]+)/page(?P<page>[0-9]+)?/$', b24online.News.views.NewsList.as_view(),
    ...
    url(r'^(?P<slug>[a-zA-z0-9-]+)-(?P<item_id>[0-9]+)\.html$', b24online.News.views.NewsDetail.as_view(),
        name="detail"),

    url(r'^tabs/gallery/(?P<item>[0-9]+)/$', b24online.News.views.NewsGalleryImageList.as_view(), name="tabs_gallery"),
    url(r'^tabs/gallery/(?P<item>[0-9]+)/page(?P<page>[0-9]+)/$', b24online.News.views.NewsGalleryImageList.as_view(),
        name="tabs_gallery_paged"),
    url(r'^tabs/gallery_structure/(?P<item>[0-9]+)/page(?P<page>[0-9]+)/$',
        b24online.News.views.NewsGalleryImageList.as_view(is_structure=True), name="gallery_structure"),
    url(r'^tabs/gallery/(?P<item>[0-9]+)/remove/(?P<pk>[0-9]+)/$',
        b24online.News.views.DeleteNewsGalleryImage.as_view(), name="gallery_remove_item")
]
